...
int main(int argc, char** argv) {
  ...
  vector<vector<double>> min_dist(nv, vector<double>(nv, 0));
  for (int i = 0; i < nv; i++) {
    for (int j = 0; j < nv; j++) {
      if (i != j) {
        for (int k = 0; k < nv; k++) {
          if (has_edge[i][k]) {
            double a = max_dist[k][j];
            double b = sqrt((vertices[i] - vertices[k]).abs2() * (1.0 - eps / EPS_COEF));
            if (a < b) {
              min_dist[i][j] = max(min_dist[i][j], b - a);
            }
          }
        }
      }
  ...
  for (int i = 0; i < nv; i++) {
    ...
    for (const auto& tp : inner) {
      ...
      for (int j = 0; j < nv && ok; j++) {
        ...
        if (dist < min_dist[i][j] * min_dist[i][j] - 1e-9) {
          ok = false;
          break;
        }
      }      
