...
/*  ifstream in_ans("../outputs/" + to_string(xid) + ".ans");
  ...
  while (true) {
    ...
    for (int i = 0; i < np; i++) {
      if (mt[i] == -1) {
        for (int j = 0; j < nv; j++) {
//          if (j == 109) continue; // !! uncomment this for test 90 border
          if (v[j].x == -1) {
            int III = i;
...
//              if ((int) max_path.size() >= 11) return;
    ...
    int L = 0;
    int R = (int) max_path.size() - 1;
    while (L <= R) {
      if (L <= R) {
        to_remove.push_back(max_path[L++]);
      }
      if (L <= R) {
        to_remove.push_back(max_path[R--]);
      }
    }
    reverse(to_remove.end() - max_path.size(), to_remove.end());
  }
 
/*  debug((poly[11] - poly[18]).abs2());
  debug(max_dist[0][3]);*/
  debug(v);
  ...
  for (int i = 0; i < nv; i++) {
    for (int j = i + 1; j < nv; j++) {
      if (v[i].x != -1 && v[j].x != -1) {
        int new_len = (v[i] - v[j]).abs2();
        assert(new_len <= max_dist[i][j] * max_dist[i][j] + 1e-9);
        if (has_edge[i][j]) {
...
/*          if (test[ii] != -1 && i != test[ii]) {
    ...
    int id = to_remove.back();
    v[id] = Point(-1, -1);
    debug("oops... fail, removing a point", id);
    to_remove.pop_back();
  }
